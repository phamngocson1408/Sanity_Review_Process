...
For filters generated by `AUTO` or `FIELDS`, a `Statement` value receives special handling. The script removes whitespace at the beginning and end, preserves whitespace inside the statement, adds `*` at both ends, and uses the Tcl `=~` operator. Existing leading or trailing wildcards are not duplicated. For example:

```text
Statement: "        if (enable) begin   "
```
...
This avoids depending on report indentation when matching a statement. `CUSTOM` expressions are emitted unchanged and do not receive this automatic transformation.
...
Owner Action = WAIVED
waiver_source_file is empty or its basename is vc_waiver.tcl
```
...
The script owns only `vc_waiver.tcl`. Waived issues reported as belonging to another waiver file, such as `rtl_pragma_waiver.tcl`, are not imported into the review workbook from the waived report and are never exported to `vc_waiver.tcl`. This prevents one waiver flow from overwriting or duplicating rules managed by another file.

The generated Tcl follows the GUI-style waiver file:

```text
one waive_violation command per unique tag/filter/comment combination
...
Rows with the same tag, resulting filter expression, and owner comment are emitted as one `waive_violation` command. The first row is the primary issue and supplies the waiver name. The primary and repeated rows use the same `waiver_name`, and Excel Notes on their `waiver_name` cells identify the shared rule relationship. Rows that differ in any of those three values generate separate commands, even if their preferred generated names initially collide; the script makes emitted names unique.
...
Statement match generated by `AUTO` or `FIELDS`:
