...
def cmd_merge_excel(args: argparse.Namespace) -> None:
    if review_workbook_has_unexported_changes(args.excel, args.waiver_tcl) and not args.force:
        raise SystemExit(
            "WARNING: lint_review.xlsx does not match the workbook recorded by gen_waiver.\n"
            "Running merge_excel now could overwrite reviewer changes.\n"
...
def review_workbook_has_unexported_changes(excel: Path, waiver_tcl: Path | None) -> bool:
    """Return whether the workbook differs from the one used by gen_waiver."""
    if not excel.exists():
        return False
    if waiver_tcl is None or not waiver_tcl.exists():
        return True
    match = re.search(
        rf"^{re.escape(WORKBOOK_HASH_PREFIX)}([0-9a-f]{{64}})$",
        waiver_tcl.read_text(encoding="utf-8", errors="replace"),
        re.MULTILINE,
    )
    if not match:
        # Waivers from older versions have no reliable link to the workbook.
        # Require one gen_waiver run rather than risk destroying reviewer edits.
        return True
    return match.group(1) != hashlib.sha256(excel.read_bytes()).hexdigest()


def stamp_waiver_workbook_hash(waiver_tcl: Path, excel: Path) -> None:
    """Record the exact review workbook represented by a generated waiver."""
    digest = hashlib.sha256(excel.read_bytes()).hexdigest()
    content = waiver_tcl.read_text(encoding="utf-8")
    marker = f"{WORKBOOK_HASH_PREFIX}{digest}\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--force",
        action="store_true",
        help="Overwrite lint_review.xlsx even when it has changes newer than vc_waiver.tcl.",
    )
