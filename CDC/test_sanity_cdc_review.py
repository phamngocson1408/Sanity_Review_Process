...
from sanity_cdc_review import (
    apply_existing_cdc_waivers,
    auto_filter_fields,
...
class CurrentDataTests(unittest.TestCase):
    def test_report_workbook_has_all_error_and_warning_issues(self):
        rows = collect_current_rows_from_report_excel(BASE_DIR / "report_cdc.full.xlsx")
        self.assertEqual(len(rows), 39)
        self.assertEqual(len({row["issue_id"] for row in rows}), 39)
        self.assertEqual(
            Counter(row["tag"] for row in rows),
            Counter({
                "CDC_COHERENCY_MULTI_SYNC": 12,
                "CDC_COHERENCY_RECONV_COMB": 9,
                "CDC_COHERENCY_RECONV_SEQ": 7,
                "SETUP_LIBCELLPIN_UNMODELLED": 6,
                "SETUP_RESET_CONSTANT_ACTIVE": 3,
                "CDC_UNSYNC_DATA": 1,
                "INTEGRITY_ASYNCRESET_COMBO_MUX": 1,
            }),
        )

    def test_reference_file_migrates_current_cdc_waivers_only(self):
        rows = collect_current_rows_from_report_excel(BASE_DIR / "report_cdc.full.xlsx")
        apply_existing_cdc_waivers(rows, BASE_DIR / "vc_waiver.tcl_ori")
        waived = [row for row in rows if row["owner_action"] == "WAIVED"]
        self.assertEqual(len(waived), 10)
    ...
    def test_generated_workbook_round_trip(self):
        rows = read_review_workbook(BASE_DIR / "outputs" / "cdc_review.xlsx")
        self.assertEqual(len(rows), 39)
        self.assertEqual(sum(row["owner_action"] == "WAIVED" for row in rows), 10)


class WaiverFormatTests(unittest.TestCase):
    def test_reference_parser_ignores_multiline_non_cdc_tail(self):
        rules = parse_waiver_tcl(BASE_DIR / "vc_waiver.tcl_ori")
        cdc_rules = [rule for rule in rules.values() if str(rule.get("app", "")).strip() == "cdc"]
        self.assertEqual(len(cdc_rules), 44)

    def test_generation_uses_cdc_app_and_preserves_custom_filter(self):
        row = {
            "issue_id": "one", "record_status": "NEW", "owner_action": "WAIVED",
            "owner_comment": "reviewed", "filter_mode": "CUSTOM",
            "custom_filter": '(DestObject == "u_sync/D")', "filter_fields": "",
            "tag": "CDC_UNSYNC_DATA", "violation": "CDC:99", "waiver_name": "CDC_UNSYNC_DATA_99",
            "waiver_user": "", "waiver_timestamp": "", "waiver_source_file": "vc_waiver.tcl",
            "fields_json": '{"Tag":"CDC_UNSYNC_DATA","DestObject":"u_sync/D"}',
        }
        with tempfile.TemporaryDirectory() as temp_dir:
            output = Path(temp_dir) / "vc_waiver.tcl"
            with patch.dict("os.environ", {"USERNAME": "cdc_user"}):
                generate_waiver_from_rows([row], output)
            generated = output.read_text(encoding="utf-8")
            parsed = parse_waiver_tcl(output)
        self.assertIn("-app { cdc }", generated)
        self.assertIn('-filter {(DestObject == "u_sync/D")}', generated)
        self.assertEqual(len(parsed), 1)
        self.assertEqual(next(iter(parsed.values()))["app"], "cdc")

    def test_qualified_cdc_filter_fields_round_trip(self):
        fields = {
            "ReasonInfoList:ReasonInfo:ReasonCode": "SYNC_SRC_CONV",
            "ConvergencePoint": "u_top/r_data/D",
        }
...
class AutoAndMergeTests(unittest.TestCase):
    ...
    def row(issue_id, convergence):
        return {
            "issue_id": issue_id, "record_status": "NEW", "tag": "CDC_RECONV",
            "module": "top", "file": "top.sv", "line": "10", "object": convergence,
            "fields_json": (
                '{"Tag":"CDC_RECONV","Module":"top","FileName":"top.sv",'
                f'"LineNumber":"10","ConvergencePoint":"{convergence}"}}'
            ),
            "owner_action": "UNREVIEWED", "reviewer_decision": "PENDING",
        }

    def test_auto_excludes_filename_and_line_and_reaches_cdc_object(self):
        ...
        self.assertEqual(selected["ConvergencePoint"], "u_top/a/D")
