...
DEFAULT_WAIVER_USER = "sanity_cdc_review"
...
def issue_id(fields: dict[str, str]) -> str:
    ignored = {
        ...
        *REVIEW_SHEET_COLUMNS, *MANAGEMENT_COLUMNS,
    }
    payload = "|".join(
        f"{name}={norm(fields.get(name))}"
        for name in sorted(fields)
        if name not in ignored and fields.get(name) is not None
    )
...
def parse_waiver_tcl(path: Path) -> dict[str, dict[str, object]]:
    ...
    for lineno, line in enumerate(path.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
        if not line.strip().startswith("waive_violation"):
            continue
        # The shared reference file may also contain multiline LINT waivers.
        # CDC commands in the supported GUI format are complete on one line.
...
def fields_for_filter(fields: dict[str, str]) -> OrderedDict[str, str]:
    ...
    priority = list(dict.fromkeys(FILTER_PRIORITY + list(fields)))
    for key in priority:
        if key in {"Tag", "Description", "Violation", "FileName", "LineNumber", "No."}:
            continue
...
def auto_filter_fields(row: dict[str, str], rows: list[dict[str, str]]) -> OrderedDict[str, str]:
    ...
    excluded = {"Tag", "Description", "Violation", "FileName", "LineNumber", "No."}
    preferred = ["Goal", "Module", "ContainerInstance"]
    priority = list(dict.fromkeys(preferred + AUTO_FILTER_PRIORITY + list(fields)))
    for key in priority:
        if key in excluded:
            continue
        value = filter_value(key, fields.get(key))
...
def audit_waiver_rules(waiver_tcl: Path, rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    for name, rule in sorted(rules.items()):
        if norm(str(rule.get("app", ""))).lower() != "cdc":
            continue
...
def comments_xml(comments: list[tuple[str, str, int]]) -> str:
    ...
    return (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<comments xmlns="{NS_MAIN}"><authors><author>sanity_cdc_review.py</author></authors>'
...
def collect_current_rows_from_report_excel(report_excel: Path) -> list[dict[str, str]]:
    ...
    for sheet_name, sheet_rows in workbook_rows.items():
        ...
        for source in sheet_rows:
            ...
            if not source.get("record_status"):
                row["record_status"] = "NEW"
            rows_by_id[row["issue_id"]] = row
    return sorted(rows_by_id.values(), key=lambda r: (r["tag"], r["module"], r["file"], int(r["line"] or 0)))


def apply_existing_cdc_waivers(rows: list[dict[str, str]], waiver_tcl: Path | None) -> None:
    """Migrate matching CDC waiver decisions from the reference Tcl file."""
    if not waiver_tcl or not waiver_tcl.exists():
        return
    rules = parse_waiver_tcl(waiver_tcl)
    ...
    for name, rule in rules.items():
        if norm(str(rule.get("app", ""))).lower() != "cdc":
            continue
        number = violation_number_from_name(name)
        tag = norm(str(rule.get("tag", "")))
        if number and tag:
            by_identity[(tag, number)] = rule
    for row in rows:
        number_match = re.search(r"(?:CDC|Lint):(\d+)", row.get("violation", ""), re.I)
        if not number_match:
            continue
        rule = by_identity.get((row.get("tag", ""), number_match.group(1)))
        if not rule:
            continue
        row.update({
            "owner_action": "WAIVED",
            "waiver_enabled": "yes",
            "waiver_name": norm(str(rule.get("add", ""))),
            "owner_comment": norm(str(rule.get("comment", ""))),
            "filter_mode": "CUSTOM",
            "custom_filter": str(rule.get("filter", "")).strip(),
            "waiver_user": norm(str(rule.get("user", ""))),
            "waiver_timestamp": norm(str(rule.get("timestamp", ""))),
            "waiver_source_file": OWNED_WAIVER_FILENAME,
        })


def violation_number_from_name(name: str) -> str:
...
def export_review_workbook(rows: list[dict[str, str]], xlsx_path: Path, summary_path: Path | None = None, report_excel: Path | None = None, previous_excel: Path | None = None) -> None:
    ...
    sheets["Instructions"] = [
        ["Sanity CDC Review Workbook"],
        ["The workbook is merged from report_cdc.full.xlsx."],
...
def generate_waiver_from_rows(rows: list[dict[str, str]], output_path: Path, user: str = "") -> None:
    lines = [
        "# Generated by sanity_cdc_review.py",
        "# Source: cdc_review.xlsx",
        "",
    ]
    eligible_rows = [
        row
    ...
    for (tag, filter_expression, comment), shared_rows in grouped_rows.items():
        ...
        lines.append(
            ...
            f"{filter_part} -app {{ cdc }} -tag {{ {tcl_escape(tag)} }} "
...
def cmd_gen_waiver(args: argparse.Namespace) -> None:
    ...
    print("Next: run the sanity tool to refresh report_cdc.full.xlsx.")


def cmd_merge_excel(args: argparse.Namespace) -> None:
    current = collect_current_rows_from_report_excel(args.report_excel)
    apply_existing_cdc_waivers(current, args.waiver_tcl)
    previous_excel = args.excel if args.excel.exists() else None
    if previous_excel:
        current = merge_rows(read_review_workbook(previous_excel), current)
    ...
    if args.waiver_tcl and args.waiver_audit:
        audit_waiver_rules(args.waiver_tcl, current, args.waiver_audit)
    print(f"review rows : {len(current)}")
    print(f"report excel: {args.report_excel}")
    print(f"excel       : {args.excel}")
    print(f"summary     : {args.summary}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage CDC sanity review with cdc_review.xlsx and waiver Tcl.",
    ...
    p = sub.add_parser("gen_waiver", help="Import cdc_review.xlsx and generate vc_waiver.tcl only.")
    p.add_argument("--excel", type=Path, default=Path("outputs/cdc_review.xlsx"))
    p.add_argument("--output", type=Path, default=Path("vc_waiver.tcl"))
    p.add_argument("--user", default="")
    p.set_defaults(func=cmd_gen_waiver)

    p = sub.add_parser("merge_excel", help="Merge report_cdc.full.xlsx into cdc_review.xlsx.")
    p.add_argument("--report-excel", type=Path, default=Path("report_cdc.full.xlsx"))
    p.add_argument("--excel", type=Path, default=Path("outputs/cdc_review.xlsx"))
    p.add_argument("--summary", type=Path, default=Path("outputs/cdc_summary.csv"))
    p.add_argument("--waiver-tcl", type=Path, default=Path("vc_waiver.tcl_ori"), help="Migrate matching existing CDC waiver decisions.")
