...
DEFAULT_WAIVER_USER = "sanity_rdc_review"
...
def parse_waiver_tcl(path: Path) -> dict[str, dict[str, object]]:
    ...
    for lineno, line in enumerate(path.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
        ...
        # RDC commands in the supported GUI format are complete on one line.
...
def audit_waiver_rules(waiver_tcl: Path, rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    for name, rule in sorted(rules.items()):
        if norm(str(rule.get("app", ""))).lower() != "rdc":
            continue
...
def comments_xml(comments: list[tuple[str, str, int]]) -> str:
    ...
    return (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        f'<comments xmlns="{NS_MAIN}"><authors><author>sanity_rdc_review.py</author></authors>'
...
def apply_existing_rdc_waivers(rows: list[dict[str, str]], waiver_tcl: Path | None) -> None:
    """Migrate matching RDC waiver decisions from the reference Tcl file."""
    ...
    for row in rows:
        number_match = re.search(r"(?:RDC|Lint):(\d+)", row.get("violation", ""), re.I)
        if not number_match:
            continue
...
def export_review_workbook(rows: list[dict[str, str]], xlsx_path: Path, summary_path: Path | None = None, report_excel: Path | None = None, previous_excel: Path | None = None) -> None:
    ...
    sheets["Instructions"] = [
        ["Sanity RDC Review Workbook"],
        ["The workbook is merged from report_rdc.full.xlsx."],
...
def generate_waiver_from_rows(rows: list[dict[str, str]], output_path: Path, user: str = "") -> None:
    lines = [
        "# Generated by sanity_rdc_review.py",
        "# Source: rdc_review.xlsx",
        "",
    ]
    eligible_rows = [
        row
    ...
    for (tag, filter_expression, comment), shared_rows in grouped_rows.items():
        ...
        lines.append(
            ...
            f"{filter_part} -app {{ rdc }} -tag {{ {tcl_escape(tag)} }} "
...
def cmd_gen_waiver(args: argparse.Namespace) -> None:
    ...
    print("Next: run the sanity tool to refresh report_rdc.full.xlsx.")


def cmd_merge_excel(args: argparse.Namespace) -> None:
    current = collect_current_rows_from_report_excel(args.report_excel)
    apply_existing_rdc_waivers(current, args.waiver_tcl)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage RDC sanity review with rdc_review.xlsx and waiver Tcl.",
    ...
    p = sub.add_parser("gen_waiver", help="Import rdc_review.xlsx and generate vc_waiver.tcl only.")
    p.add_argument("--excel", type=Path, default=Path("outputs/rdc_review.xlsx"))
    p.add_argument("--output", type=Path, default=Path("vc_waiver.tcl"))
    p.add_argument("--user", default="")
    p.set_defaults(func=cmd_gen_waiver)

    p = sub.add_parser("merge_excel", help="Merge report_rdc.full.xlsx into rdc_review.xlsx.")
    p.add_argument("--report-excel", type=Path, default=Path("report_rdc.full.xlsx"))
    p.add_argument("--excel", type=Path, default=Path("outputs/rdc_review.xlsx"))
    p.add_argument("--summary", type=Path, default=Path("outputs/rdc_summary.csv"))
    p.add_argument("--waiver-tcl", type=Path, default=Path("vc_waiver.tcl_ori"), help="Migrate matching existing RDC waiver decisions.")
